...
<div class="row" hidden id="panelLeccion">
	<div class="col-md-8" style="padding-right:0.2%; padding-left:0.2%">
		...
		<div class="col-md-12">
			<div @if(Auth::user()->discapacidad == 'ceguera') hidden @endif id=panelVideo class="panel panel-default" style="border-color: #2C3E50">
	...
	<div class="col-md-4" style="padding-right:0.9%; padding-left: 0.2%">
		...
		<div class="col-md-12" >
			...
			{!! Form::open(['route' => ['member.comentarios.store', 'leccion_id' => $leccion->id, 'num' => $num, 'slug' => $leccion->slug ], 'method' => 'POST']) !!}

			<div class="form-group col-md-12 no-padding">
				{!! Form::label('descripcion','¡Añade un comentario! (Máximo 255 caracteres)') !!}
...
{{-- Comentarios Script petición por Ajax--}}
<script type="text/javascript">
	...
	$.ajax({
		dataType: 'json',
		data: dataEnviar,
		url: '{{ route('member.comentarios.show',$leccion->id) }}',
		type: 'get',
		success:  function (response) {
			...
			for(var i=0; i<response.length; i++){
				...
				if(response[i].user_id == {{Auth::user()->id}}){
					$propio = true;
				}

				agregarComentario($comentario,$username,$imagen,$coment_id,$dateCarbon,$propio);
			}
	...
	function agregarComentario(comentario,username,imagen,coment_id,dateCarbon,propio){
		...
		if(propio){
			toAppend += '<div class="col-xs-3" style="padding-right: 2px; margin-bottom:2px"><button title="Eliminar" type="button" onclick="eliminarComentario(this)" value="'+coment_id+'" class="btn  btn-xs btn-primary pull-right boton-eliminar" style="background-color:#BB0515;margin-right: 5px; margin-top: 0; padding-left: 4px;padding-right: 4px;">Eliminar comentario<span class="glyphicon glyphicon-trash"></span></button></div>';
		}

		toAppend += '</div>'; //Ultimo div necesario
...
<script> {{-- Y O U T U B E           A P I --}}
  ...
  {{-- Guardar eventos de Youtube Script Ajax --}}
  <script type="text/javascript">
